...
export class Maker {
  private readonly _maps: Map<string | symbol, IMap> = new Map<
    string | symbol,
    IMap
  >();
  ...
  make<T extends any>(constructor?: ClassType<T>, pool?: any): T {
    const made: T | any =
      constructor === null || constructor === undefined
        ? {}
        : new constructor();
    ...
    keys.forEach((key) => {
      ...
      let type: ClassType<any> =
        map && map.type
          ? map.type
          : Reflect.getMetadata("design:type", made, key);
      if (!type && pool) {
      ...
      if (value !== undefined || (map && map.undefined)) {
        made[property] =
          type && map && map.nested && value
            ? this.makeNested(type, value)
            : value;
      }
    });
  ...
  private makeNested<T>(constructor: ClassType<T>, pool?: any): T {
    let maker: Maker | undefined =
      pool && pool.constructor
        ? Reflect.getMetadata(MakeItKey, pool.constructor)
        : undefined;
    if (maker === undefined)
      maker = constructor
        ? Reflect.getMetadata(MakeItKey, (new constructor() as any).constructor)
        : undefined;
