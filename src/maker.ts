...
export class Maker {
    ...
    make<T extends any>(constructor?: ClassType<T>, pool?: any): T {
        ...
        keys.forEach((key) => {
            ...
            if (value !== undefined || map && map.undefined) {
                made[property] = type && map && map.nested && value
    ...
    private makeNested<T>(constructor: ClassType<T>, pool?: any): T {
        let maker: (Maker | undefined) = pool && pool.constructor ? Reflect.getMetadata(MakeItKey, pool.constructor) : undefined;
        if (maker === undefined) maker = constructor ? Reflect.getMetadata(MakeItKey, (new constructor() as any).constructor) : undefined;
